...
function getDifficultyConfig(diff, verse) {
  ...
  const timeLimit = TIME_LIMITS[diff] || 80;

  if (diff === "easy") {
    // Easy: 60 seconds, 50% on board, 50% to be added, 1 incorrect word
    ...
    // Medium: 80 seconds, phrase-by-phrase, initial word on board for each phrase, 1 incorrect word per phrase
    ...
    // Hard: 100 seconds, complete whole verse, 0 revealed, 2 incorrect words
    return {
      type: "hard",
      timeLimit: 100,
      prefilledIndices: [],
